...
function Indoor(){
  ...
  const load = async ()=>{
    const r1 = await fetch(`${BASE}/api/matches?sport=indoor&status=live`)
    const r2 = await fetch(`${BASE}/api/matches?sport=indoor&status=upcoming`)
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      <h1 className="text-3xl font-bold">GBU Indoor Stadium</h1>
      <p className="text-gray-600 mt-2">Badminton, Table Tennis, and more.</p>
      <div className="mt-6 grid md:grid-cols-2 gap-6">
        <div>
          <h2 className="text-xl font-semibold mb-2">Live Matches</h2>
          <div className="space-y-3">
            {live.length ? live.map(m=> (
              <div key={m.id} className="p-4 rounded-lg border bg-white">
                <div className="text-sm text-gray-500">{m.status?.toUpperCase()}</div>
